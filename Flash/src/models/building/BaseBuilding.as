package models.building
{
	...
	import models.away3D.buildings.Tower3D;
	...
	public class BaseBuilding implements BuildingObject, IEventDispatcher
	{
		...
		public function set atValidLocation(iVL:Boolean) : void {
			...
			if(this._atValidLocation != iVL){
				...
				var event:PropertyChangeEvent = new PropertyChangeEvent(BaseBuilding.AT_VALID_LOCATION_CHANGE, false, false,
		...
		public function statelessEqual(s:BuildingObject):Boolean
		{
			if(this.isObject(s)){
		...
		public function isEqual(s:BuildingObject):Boolean
		{
			return false;
		}
		...
		public function onDragStart(event:MapMouseEvent) : void {
			...
			var e:PropertyChangeEvent = new PropertyChangeEvent(BaseBuilding.ON_DRAG_START, false, false, 
		...
		public function onDragEnd(event:MapMouseEvent) : void {
			...
			// Dispatch that we have begun dragging the tower
			var e:PropertyChangeEvent = new PropertyChangeEvent(BaseBuilding.ON_DRAG_END, false, false, 
		...
		public function isObject(s:BuildingObject):Boolean {
			return false
		}
